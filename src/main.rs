...
fn main() {
    ...
    let (tx_in, rx_in) = mpsc::sync_channel::<Block>(100);
    let (tx_out, rx_out) = mpsc::sync_channel::<Block>(100);
    ...
    let reader_thread = thread::spawn(move || {
        let file = File::open("test.bin").expect("There was an error reading the file");
        let file_size = file
            .metadata()
        ...
        while read_bytes_left > 0 {
            let to_read: usize = cmp::min(read_bytes_left as usize, 2048);
            let mut data = vec![0; to_read];

            println!("Reading the {} block with a size of {}", id, to_read);
            let _ = buf_reader.read_exact(&mut data);
